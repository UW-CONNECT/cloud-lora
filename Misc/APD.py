...
import Client.DWT_compress as DWT
...
class Active_Period_Detector2:
    ...
    def Active_Sess_Detect2(self, x_1, level, tt):
        ...
        if self.curr_state > 0:
            self.low_buffer = self.win_jump_factor
            self.curr_state = 3
            sess_mag = np.mean(
            ...
            uplink_wind.append(
                [(pot_wind_start - self.front_buf), int(len(x_1) / win_jump) + UPSAMPLE_FACTOR, 1,
                 np.log10(sess_mag), mini_hist, dden, tt])

        if len(uplink_wind) > 0 and uplink_wind[0][0] < 1:  # need to add fragmented flag
            uplink_wind[0][0] = 0
            uplink_wind[0][6] = tt
