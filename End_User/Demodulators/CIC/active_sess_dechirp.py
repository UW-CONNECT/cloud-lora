...
def active_sess_dechirp(x_1, SF, BW, Fs):
    ...
    mov_thresh_rec = []
    for i in range(math.floor(len(x_1)/win_jump) - win_jump_factor):
        wind = x_1[i * win_jump: i * win_jump + (N * upsampling_factor)]
        wind_fft = np.abs(np.fft.fft(wind * DC_upsamp))
        wind_fft = wind_fft[np.concatenate([np.arange(N//2, dtype=int), np.arange((N//2 + (upsampling_factor-1)*N),
                                                                                  upsampling_factor * N, dtype=int)])]
        noise_floor = np.mean(wind_fft)
        n.append(noise_floor)
        fft_peak = wind_fft.max()
    ...
    if len(uplink_wind) > 0 and uplink_wind[-1, 1] > length(x_1):
