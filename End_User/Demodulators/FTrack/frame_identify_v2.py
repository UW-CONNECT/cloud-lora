from .config import *
from .frame_preamble_identify import frame_preamble_identify
from .preamb_cluster import preamb_cluster
from .frame_sync_words_search import frame_sync_words_search
...
from .preamb_reconstruct import preamb_reconstruct
from .preamb_corr_detect_v2 import preamb_corr_detect_v2
from .preamb_edge_detect_v2 import preamb_edge_detect_v2
from .preamb_edge_fitting import preamb_edge_fitting


def frame_identify_v2(preambles, preamb_idxes, trk_user_map, raw_signals, ftrack_pwr, ftracks, chirp_len, fft_size, pwr_thresholds, SF):
    ...
    for s in range(seg_num):
        ...
        lcal_threshold = local_pwr_threshold(np.concatenate(([seg_st], [seg_ed])), pwr_thresholds, SF)
        
        # extract symbol edges
        edges = edge_detect_coef_comp(preamb_signal_seg, fft_size, lcal_threshold, SF)
    ...
    preambs_dedup, preamb_trk = preamb_cluster(pamb_edges, pamb_trks, ftrack_pwr, SF)
...
def edge_detect_coef_comp(preamb_signal_seg, num_symb, pwr_threshold, SF):

    # parameter configuration
    FTRACK_LEN_MIN = np.floor(2*TRK_SPAN_RATIO*(int(2)**SF))
    PREAMB_CHIRPS_MIN = TRK_PREAMBLE_MIN
...
def edges_at_preamble(preamb_signal_seg, pamb_trk, num_symb, nfft):
    ...
    # detect chirp edges
    corr_threshold = EDGE_CORRELATION_THRESHOLD
...
def local_pwr_threshold(thresh_range, thresholds, SF):
