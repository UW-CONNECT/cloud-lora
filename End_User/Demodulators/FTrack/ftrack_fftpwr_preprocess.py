import numpy as np
from .config import *
from .util_fftpwr_peaks import util_fftpwr_peaks


def ftrack_fftpwr_preprocess(fft_pwr, SF):
    ...
    min_peak_dist = AUTO_PEAK_DIST
    pwr_diff_threshold = AUTO_NOISE_FLOOR
    ...
    for col in np.arange(0, col_num, TIME_WIN):
        ...
        for fbin in np.arange(0, bin_num, FBIN_WIN):
            ...
            [pwr_trks, local_threshold] = util_fftpwr_2D_filter(pwr_subcell, min_peak_dist, pwr_diff_threshold, SF)
...
def util_fftpwr_2D_filter(fpwr, min_peak_dist, pwr_diff_threshold, SF):

    sz = fpwr.shape
    col_num = sz[1]
    
    # load parameter from the configuration file
    PEAK_SAMPLE_STEP = 2**(SF-4)
    HIGH_PEAK_RATIO = AUTO_HIGH_PEAK_RATIO
    PEAK_PWR_RATIO = AUTO_PEAK_PWR_RATIO
    NOISE_FLOOR = AUTO_NOISE_FLOOR
