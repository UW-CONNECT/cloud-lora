from .config import *
from .util_track_deduplicate import util_track_deduplicate


def ftrack_grouping_v2(ftracks, user_borders, track_user_map, edge_trk_map, chirp_len, ftrack_pwr, SF):
    ...
    for u in range(user_num):
        ...
        user_id = u + 1
        [track_user_map, track_edge_map] = ftracks_2_user(sorted_tracks, edges, track_user_map, track_edge_map, user_id, chirp_len, ftrack_pwr, SF)
...
def ftracks_2_user(ftracks, edges, track_user_map, track_edge_map, user_id, chirp_len, ftrack_pwr, SF):
    ...
    for e in range(edge_num):
        ...
        track_id = track_select(ftracks, dedup_candies, edge_line, ch_range, ftrack_pwr, SF)
...
def track_select(ftracks, cand_indexes, edge_line, chirp_len, ftrk_pwr, SF):

    cand_num = cand_indexes.size
    
    EDGE_RANGE = -1 * np.floor(0.01*(int(2)**SF))
    ...
    use_pwr_metric = (FTRK_GROUP_ALPHA < 1)
...
def track_edge_relation(track, edge, chirp_len):
    ...
    RANGE_THRESHOLD = TRK_SPAN_RATIO
