...
def process_data(file_from, respQ):
    ...
    file, addr = file_from
    pkt_size = int.from_bytes(file[:4], 'big')
    ...
    IQ = np.frombuffer(file[24:int(pkt_size + 4)], dtype=np.complex64)
    ...
    for SF in LORA_SF:
       pkts = demoder.Evaluate(IQ, SF, LORA_BW, bs_fs[0], True)
...
if __name__ == "__main__":
    ...
    myPool = multiprocessing.Pool(1, worker, (sessions, responses))
